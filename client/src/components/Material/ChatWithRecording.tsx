import {
  ...
  const ChatWithRecording = ({roomId}:{roomId:string}) => {
    ...
    const handleMessageSend = async () => {
      ...
      const response = await fetch(
        `http://127.0.0.1:5000/api/chat-with-recording/${roomId}`,
        {
          method: "POST",
          headers: {
    ...
    return (
      <Stack p="lg" w="100%" h='100%' mah="100%" style={{border:'1px solid black',borderRadius:'10px'}}>
        <Title size="h4">Chat With Recording</Title>
        <Stack
          w="100%"
          mah="100%"
  ...
  export default ChatWithRecording;
